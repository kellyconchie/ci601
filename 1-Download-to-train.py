...
if not isExist:
    os.makedirs('../csv/train')
    os.makedirs('../csv/train/GBP')
    os.makedirs('../csv/train/XCH')
...
dfor.to_csv('../csv/train/forex.csv', index=False)

# set dates for download
# run date from Sunday to Saturday to catch all time zones
# 2 weeks to get moving average data and 2 weeks to train
...
while i < rn:
    ...
    del df2['High']
    del df2['Low']
    df2.to_csv('../csv/train/GBP/' + mquote + gbp + '=x.csv')
    i = i + 1
...
while i < rn:
    ...
    while ii < rn:
        ...
        if rn2 > 5:
            df2.to_csv('../csv/train/XCH/' + mbase + mquote + '=x.csv')
            df3 = pd.read_csv('../csv/train/forex.csv')

            # check if file name already exists in the forex file. If not then add it
            if tic not in df3.values:
                row = [tic, cur]
                df3.loc[len(df3)] = row
                df3.to_csv('../csv/train/forex.csv', index=False)
        ii = ii + 1
    i = i + 1
...
while i < rn:
    mquote = df.loc[i, 'code']
    mtic = mquote + gbp
    if os.path.isfile('../csv/train/GBP/' + mtic + "=x.csv"):
        dff = pd.read_csv('../csv/train/GBP/' + mtic + "=x.csv")
        rn2 = len(dff)
        print(mtic)
        if rn2 < 5:
            row = [mtic]
            dpc.loc[len(dpc)] = row
            dpc.to_csv('../csv/gbpcheck.csv', index=False)
            os.remove('../csv/train/GBP/' + mtic + "=x.csv")
            print(mtic + " removed")
    else:
        row = [mtic]
        dpc.loc[len(dpc)] = row
...
while i < rn:
    ...
    mt2 = str(mtc)[0:3]
    dfor = pd.read_csv('../csv/train/forex.csv')
...
while i < ren:
    ...
    if os.path.isfile('../csv/train/XCH/' + mtic + '.csv'):
        os.remove('../csv/train/XCH/' + mtic + '.csv')
...
df = pd.read_csv('../csv/train/forex.csv')
rn = len(df)
i = 0
while i < rn:
    mtic = df.loc[i, 'ticker']
    df2 = pd.read_csv('../csv/train/XCH/' + mtic + '.csv')
    if 'GBPquote' in df2.columns:
        print(' ')
    else:
        df2.insert(2, 'GBPquote', 1)
        df2.insert(3, 'Realclose', 1)
    df2.to_csv('../csv/train/XCH/' + mtic + '.csv', index=False)
...
while i < rn:
    ...
    df3 = pd.read_csv('../csv/train/GBP/' + mtic2)
    lendf3 = len(df3)
    i2 = 0
    while i2 < lendf3:
